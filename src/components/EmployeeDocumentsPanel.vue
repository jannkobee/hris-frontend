<template>
  <div class="employee-documents-panel">
    <v-expand-transition>
      ...
      <article v-for="document in documents" :key="document.id" class="document-row">
        ...
        <div class="document-copy">
          <div class="d-flex align-center ga-2 flex-wrap">
            ...
            <v-chip
              v-if="document.visibility === 'hr_only'"
              size="x-small"
              color="warning"
              variant="tonal"
...
const upload = async () => {
  if (!normalizedFile.value || !props.employee?.id) return;
...
watch(
  [() => props.active, () => props.employee?.id],
  ([active, employeeId]) => active && employeeId && load(),
  { immediate: true },
);
</script>
