    ...
    <v-menu location="bottom end" :close-on-content-click="false" max-width="420">
      <template #activator="{ props }">
        ...
        >
          ...
          <span v-if="notificationCount" class="notification-badge">{{ displayBadge(notificationCount) }}</span>
        </v-btn>
      </template>
      <v-card class="notification-menu" min-width="360">
        <v-card-title class="d-flex align-center py-3">
          <div><div class="text-subtitle-1 font-weight-bold">Notifications</div><div class="text-caption text-medium-emphasis">{{ notificationCount ? `${notificationCount} unread` : "You're all caught up" }}</div></div>
          <v-spacer />
          <v-btn size="small" variant="text" @click="router.push({ name: 'notifications' })">View all</v-btn>
        </v-card-title>
        <v-divider />
        <v-list lines="two" max-height="420" class="overflow-y-auto py-0">
          ...
          >
            <template #prepend><v-avatar size="34" color="primary" variant="tonal"><v-icon icon="mdi-bell-outline" size="18" /></v-avatar></template>
          </v-list-item>
          <v-list-item v-if="!notifications.length" title="You are all caught up" subtitle="No notifications yet."><template #prepend><v-avatar size="34" color="success" variant="tonal"><v-icon icon="mdi-check" /></v-avatar></template></v-list-item>
        </v-list>
      </v-card>
    </v-menu>
...
.notification-menu { overflow: hidden; border-radius: 14px; }
