<template>
  <v-container fluid class="shift-roster">
    <ModuleHeader
      ...
      title="Shifts & Roster"
      subtitle="Plan work schedules, then compare them with attendance records."
    ...
    <v-tabs v-model="tab" color="primary" class="mb-4"
      ...
      ><v-tab value="templates">Shift templates</v-tab></v-tabs
    >
    <v-window v-model="tab">
      <v-window-item value="roster">
        <v-card>
          <v-card-title class="d-flex align-center flex-wrap ga-3"
            >Employee roster <v-spacer /><v-text-field
              v-model="from"
              type="date"
              label="From"
              density="compact"
          ...
          <v-table
            ...
            <tbody>
              <tr v-for="assignment in assignments" :key="assignment.id">
                ...
                <td>
                  {{ assignment.employee?.user?.full_name || "Employee" }}
                </td>
                <td>{{ assignment.shift_name }}</td>
                <td>{{ assignment.start_time }} – {{ assignment.end_time }}</td>
                <td>
                  {{ assignment.break_minutes }}m /
                  {{ assignment.grace_minutes }}m
                </td>
                <td v-if="canManage">
                  <v-btn
              ...
              <tr v-if="!assignments.length">
                ...
                >
                  No shifts are assigned in this period.
                </td>
              </tr>
            </tbody></v-table
          >
      ...
      <v-window-item value="templates">
        <v-card
          ><v-card-title class="d-flex align-center"
            >Shift templates <v-spacer /><v-btn
              v-if="canManage"
              color="primary"
              size="small"
          ...
          <v-table
            ...
            <tbody>
              <tr v-for="template in templates" :key="template.id">
                ...
                <td>
                  {{ template.break_minutes }}m / {{ template.grace_minutes }}m
                </td>
                <td>
                  <v-chip
                    size="small"
                    :color="template.is_active ? 'success' : 'default'"
                    >{{ template.is_active ? "Active" : "Inactive" }}</v-chip
                  >
                </td>
                <td v-if="canManage">
                  <v-btn
    ...
    <v-dialog v-model="templateDialog" max-width="560"
      ><v-card
        ><v-card-title
          >{{ templateForm.id ? "Edit" : "Create" }} shift
        ...
        ><v-card-actions
          ><v-spacer /><v-btn @click="templateDialog = false">Cancel</v-btn
      ...
      ></v-dialog
    >
    <v-dialog v-model="assignmentDialog" max-width="560"
      ><v-card
        ><v-card-title
        ...
        ><v-card-actions
          ><v-spacer /><v-btn @click="assignmentDialog = false">Cancel</v-btn
          ><v-btn color="primary" :loading="saving" @click="saveAssignment"
            >Save</v-btn
          ></v-card-actions
        ></v-card
      ></v-dialog
...
const removeAssignment = async (assignment: any) => {
  await axios.delete(`/shift-assignments/${assignment.id}`);
  await loadAssignments();
};
onMounted(async () => {
...
<style scoped>
.shift-roster :deep(.v-card-title) {
  padding: 16px 20px;
}
.shift-roster :deep(th) {
  white-space: nowrap;
}
</style>
