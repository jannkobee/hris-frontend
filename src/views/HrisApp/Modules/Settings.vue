<template>
  <v-container fluid class="settings-page">
    <ModuleHeader
      ...
      title="App Settings"
      subtitle="Control your workspace experience and company-wide policies."
    ...
    <template v-if="authUser && settingsReady">
      ...
      >
        ...
        <v-tab
          value="payroll"
          :disabled="!canUsePayroll"
          :title="
            canUsePayroll
              ? 'Configure payroll policies'
              : 'Payroll is not included in the current plan'
          "
        >
          <v-icon
            :icon="canUsePayroll ? 'mdi-cash-multiple' : 'mdi-lock-outline'"
            start
            size="small"
          />
          Payroll
        ...
        <v-tab
          ...
          :title="scheduledTasksAvailability"
        >
          <v-icon
            :icon="canViewTasks ? 'mdi-clock-outline' : 'mdi-lock-outline'"
            start
            size="small"
          />
          Scheduled Tasks
        ...
        <v-window-item v-if="canUsePayroll" value="payroll">
          <div class="payroll-settings-intro">
            ...
            <v-select
              ...
              :items="[
                { title: 'Philippines (currently supported)', value: 'PH' },
              ]"
              density="compact"
              variant="outlined"
              hide-details
...
const scheduledTasksAvailability = computed(() => {
  if (!hasFeature("automation")) {
    return "Automation is not included in the current plan";
  }

  if (!checkPermissions("view-scheduled-tasks")) {
    return "You do not have permission to view scheduled tasks";
  }

  return "Manage scheduled automation";
});
