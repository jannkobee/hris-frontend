      ...
      <aside class="conversation-pane">
        ...
        <div class="conversation-list">
          ...
          <button v-for="conversation in filteredConversations" v-else :key="conversation.id" type="button" class="conversation-item" :class="{ 'conversation-item--active': activeConversation?.id === conversation.id, 'conversation-item--unread': conversation.unread_count }" @click="selectConversation(conversation)">
            <UserAvatar :user="conversationAvatarUser(conversation)" :name="conversationTitle(conversation)" size="46" />
            <div class="conversation-copy"><div><strong>{{ conversationTitle(conversation) }}</strong><time v-if="conversation.latest_message?.created_at">{{ conversationListTime(conversation.latest_message.created_at) }}</time></div><p>{{ conversationPreview(conversation) }}</p></div>
      ...
      <main class="thread-pane">
        <template v-if="activeConversation">
          <header class="thread-header">
            ...
            <UserAvatar :user="conversationAvatarUser(activeConversation)" :name="conversationTitle(activeConversation)" size="42" />
            <div><strong>{{ conversationTitle(activeConversation) }}</strong><span>{{ participantSummary(activeConversation) }}</span></div>
            <v-spacer />
            <v-btn icon="mdi-information-outline" variant="text" size="small" title="Conversation information" aria-label="Conversation information" @click="openConversationInfo" />
          </header>
          ...
          <div ref="scrollArea" class="thread-body">
            ...
            <template v-for="(message, index) in messages" v-else :key="message.id">
              ...
              <div class="message-row" :class="{ 'message-row--own': isOwnMessage(message) }">
                <UserAvatar v-if="showSenderDetails(message, index)" :user="message.sender" :name="senderName(message)" color="surface-variant" variant="flat" size="30" />
                <span v-else-if="!isOwnMessage(message)" class="message-avatar-spacer" aria-hidden="true" />
                <div class="message-stack">
                  <span v-if="showSenderDetails(message, index)" class="message-sender">{{ senderName(message) }}</span>
                  ...
                  <div v-if="message.body" class="message-bubble" :class="{ 'message-bubble--own': isOwnMessage(message) }"><p>{{ message.body }}</p><time>{{ formatTime(message.created_at) }}</time></div>
                  <time v-else class="attachment-time">{{ formatTime(message.created_at) }}</time>
                </div>
              </div>
            </template>
          </div>
          ...
          <footer class="composer-wrap">
            <div v-if="selectedFiles.length" class="pending-files">
              <div v-for="(file, index) in selectedFiles" :key="`${file.name}-${file.lastModified}`" class="pending-file">
                <span class="pending-file__icon"><v-icon :icon="selectedFileIcon(file)" size="19" /></span>
                <span class="pending-file__copy"><strong>{{ file.name }}</strong><small>{{ formatSelectedFileSize(file.size) }}</small></span>
                <v-btn icon="mdi-close" variant="text" size="x-small" :aria-label="`Remove ${file.name}`" @click="removeSelectedFile(index)" />
              </div>
            </div>
            <v-progress-linear v-if="sending && uploadProgress !== null" :model-value="uploadProgress" color="primary" height="3" rounded />
            <div class="composer">
              <input ref="attachmentInput" class="d-none" type="file" multiple :accept="attachmentAccept" @change="onAttachmentInput" />
              <v-btn v-if="attachmentsEnabled" icon="mdi-paperclip" color="primary" variant="text" size="small" :disabled="sending || selectedFiles.length >= 5" title="Attach files" aria-label="Attach files" @click="triggerAttachmentPicker" />
              <v-textarea v-model="newMessageBody" :placeholder="`Message ${conversationTitle(activeConversation)}`" rows="1" auto-grow max-rows="5" density="compact" variant="plain" hide-details @paste="onComposerPaste" @keydown.enter.exact.prevent="sendMessage" />
              <v-btn icon="mdi-send" color="primary" variant="flat" size="small" :loading="sending" :disabled="!canSendMessage" aria-label="Send message" @click="sendMessage" />
            </div>
            <span>{{ attachmentsEnabled ? `Enter to send · Up to 5 files · ${maxAttachmentSizeMb} MB each` : 'Enter to send · Shift + Enter for a new line' }}</span>
          </footer>
        </template>
    ...
    <v-dialog v-model="imagePreviewDialog" max-width="980">
      <v-card class="image-preview-dialog">
        <v-card-title class="image-preview-header">
          ...
          <v-spacer />
          <v-btn icon="mdi-download-outline" variant="text" size="small" aria-label="Download image" @click="downloadImagePreview" />
          <v-btn icon="mdi-close" variant="text" size="small" aria-label="Close image preview" @click="imagePreviewDialog = false" />
        </v-card-title>
        <v-divider />
        <v-card-text class="image-preview-body">
          <v-img v-if="imagePreview" :src="imagePreview.url" :alt="imagePreview.name" max-height="76vh" contain />
        </v-card-text>
      </v-card>
    </v-dialog>
    ...
    <v-dialog v-model="conversationInfoDialog" max-width="520" scrollable>
      <v-card class="conversation-info-dialog">
        <v-card-title class="conversation-info-header">
          <div class="conversation-info-header__icon"><v-icon icon="mdi-information-outline" size="21" /></div>
          <div><strong>Conversation details</strong><small>People, activity, and shared files.</small></div>
          <v-spacer />
          <v-btn icon="mdi-close" variant="text" size="small" aria-label="Close conversation information" @click="conversationInfoDialog = false" />
        </v-card-title>
        <v-divider />
        <v-card-text class="conversation-info-body">
          <div v-if="loadingConversationInfo" class="conversation-info-loading">
            <v-skeleton-loader type="list-item-avatar-two-line, article, list-item-three-line" />
          </div>
          <template v-else-if="conversationInfo">
            <div class="conversation-info-summary">
              <UserAvatar :user="conversationAvatarUser(conversationInfo)" :name="conversationTitle(conversationInfo)" size="58" />
              <div><strong>{{ conversationTitle(conversationInfo) }}</strong><span>{{ conversationInfo.is_group ? 'Group conversation' : 'Direct conversation' }} · Started {{ formatConversationDate(conversationInfo.created_at) }}</span></div>
            </div>

            <div class="conversation-info-metrics">
              <div><v-icon icon="mdi-message-text-outline" /><strong>{{ conversationInfo.messages_count ?? 0 }}</strong><span>Messages</span></div>
              <div><v-icon icon="mdi-paperclip" /><strong>{{ conversationInfo.attachments_count ?? 0 }}</strong><span>Shared files</span></div>
              <div><v-icon icon="mdi-account-multiple-outline" /><strong>{{ conversationInfo.participants?.length ?? 0 }}</strong><span>People</span></div>
            </div>

            <section class="conversation-info-section">
              <div class="conversation-info-section__title"><strong>Participants</strong><span>{{ conversationInfo.participants?.length ?? 0 }}</span></div>
              <div class="conversation-participants">
                <div v-for="participant in conversationInfo.participants" :key="participant.id" class="conversation-participant">
                  <UserAvatar :user="participant" :name="userDisplayName(participant)" size="38" />
                  <div><strong>{{ userDisplayName(participant) }}</strong><span>{{ participant.role?.name || participant.email || 'Team member' }}</span></div>
                  <v-chip v-if="participant.id === authUser?.id" size="x-small" color="primary" variant="tonal">You</v-chip>
                </div>
              </div>
            </section>

            <section v-if="conversationInfo.recent_attachments?.length" class="conversation-info-section">
              <div class="conversation-info-section__title"><strong>Recently shared</strong><span>{{ conversationInfo.attachments_count }}</span></div>
              <div class="conversation-shared-files">
                <button v-for="attachment in conversationInfo.recent_attachments" :key="attachment.id" type="button" class="conversation-shared-file" @click="openSharedAttachment(attachment)">
                  <span><v-icon :icon="attachmentIcon(attachment)" size="20" /></span>
                  <div><strong>{{ attachment.original_name }}</strong><small>{{ formatSelectedFileSize(attachment.size) }} · {{ attachment.is_image ? 'Image' : 'File' }}</small></div>
                  <v-progress-circular v-if="openingAttachmentId === attachment.id" indeterminate size="18" width="2" />
                  <v-icon v-else :icon="attachment.is_image ? 'mdi-eye-outline' : 'mdi-download-outline'" size="18" />
                </button>
              </div>
            </section>
          </template>
...
const imagePreview = ref<{ url: string; name: string; downloadUrl: string } | null>(null);
...
const canSendMessage = computed(
  () => Boolean(newMessageBody.value.trim() || selectedFiles.value.length) && !sending.value,
);
const attachmentAccept = ".jpg,.jpeg,.png,.gif,.webp,.pdf,.doc,.docx,.xls,.xlsx,.ppt,.pptx,.txt,.csv,.zip,.rar,.7z,.mp3,.wav,.m4a,.mp4,.mov,.webm";
...
const conversationTitle = (conversation: any) => {
  ...
  return others.map((participant: any) => userDisplayName(participant)).join(", ") || "Conversation";
};
...
const conversationPreview = (conversation: any) => {
  ...
  if (!latest.body && latest.attachments?.length) {
    return latest.attachments.length === 1 ? "Sent an attachment" : `Sent ${latest.attachments.length} attachments`;
  }
  if (!latest.body) return "Start the conversation";
  return `${latest.sender?.id === authUser.value?.id ? "You: " : ""}${latest.body}`;
};
...
const showSenderDetails = (message: any, index: number) => {
  ...
  return !previous || previous.sender?.id !== message.sender?.id || showDateDivider(message, index);
};
...
const attachmentIcon = (attachment: any): string => {
  ...
  if (["zip", "rar", "7z"].includes(extension ?? "")) return "mdi-folder-zip-outline";
  return "mdi-file-document-outline";
};
...
const addSelectedFiles = (files: File[]) => {
  ...
  const valid = files.filter((file) => file.size <= maximumBytes).slice(0, remainingSlots);
  ...
  if (files.length - oversized.length > remainingSlots) {
    showNotification("Attachment limit reached", "You can send up to 5 files in one message.", "warning");
  }
  ...
  selectedFiles.value.push(
    ...valid.filter((file) => !existingKeys.has(`${file.name}:${file.size}:${file.lastModified}`)),
  );
};
...
const openImagePreview = (preview: { url: string; name: string; downloadUrl: string }) => {
...
const openConversationInfo = async () => {
  ...
  try {
    const { data } = await axios.get(`/conversations/${activeConversation.value.id}`);
...
const sendMessage = async () => {
  ...
  if ((!body && !selectedFiles.value.length) || !activeConversation.value) return;
...
.conversation-pane__header{background:linear-gradient(145deg,rgba(var(--v-theme-primary),.045),transparent 65%)}
.conversation-item{isolation:isolate;margin-bottom:3px;padding:12px 11px;overflow:hidden}
.conversation-item:before{position:absolute;top:10px;bottom:10px;left:0;width:3px;border-radius:0 999px 999px 0;background:transparent;content:"";transition:background-color .15s ease}
.conversation-item--active{background:rgba(var(--v-theme-primary),.105);box-shadow:inset 0 0 0 1px rgba(var(--v-theme-primary),.09)}
.conversation-item--active:before{background:rgb(var(--v-theme-primary))}
.conversation-item :deep(.user-avatar),.thread-header :deep(.user-avatar){box-shadow:0 0 0 2px rgb(var(--v-theme-surface)),0 0 0 3px rgba(var(--v-theme-primary),.15)}
.thread-header{min-height:74px;background:linear-gradient(90deg,rgba(var(--v-theme-primary),.04),transparent 44%)}
.thread-body{background:linear-gradient(180deg,rgba(var(--v-theme-primary),.025),transparent 25%),rgba(var(--v-theme-on-surface),.008)}
.message-avatar-spacer{width:30px;flex:0 0 30px}
.message-bubble{padding:10px 12px 8px;border-radius:7px 17px 17px;box-shadow:0 4px 14px rgba(0,0,0,.04)}
.message-bubble--own{border-radius:17px 7px 17px 17px}
.composer{min-height:48px;border-radius:15px;background:rgba(var(--v-theme-on-surface),.018)}
.new-message-header{background:linear-gradient(135deg,rgba(var(--v-theme-primary),.055),transparent 58%)}
.message-attachments{display:grid;gap:6px;margin-bottom:5px}.message-row--own .message-attachments{justify-items:end}.attachment-time{margin:2px 5px 0;color:rgb(var(--v-theme-on-surface-variant));font-size:.57rem}.message-row--own .attachment-time{align-self:flex-end}.pending-files{display:flex;gap:7px;padding:0 0 10px;overflow-x:auto}.pending-file{display:grid;min-width:190px;max-width:270px;grid-template-columns:auto minmax(0,1fr) auto;align-items:center;gap:8px;padding:7px 8px;border:1px solid rgba(var(--v-theme-on-surface),.09);border-radius:11px;background:rgba(var(--v-theme-primary),.045)}.pending-file__icon{display:grid;width:32px;height:32px;place-items:center;border-radius:9px;color:rgb(var(--v-theme-primary));background:rgba(var(--v-theme-primary),.11)}.pending-file__copy{display:flex;min-width:0;flex-direction:column}.pending-file__copy strong{overflow:hidden;font-size:.68rem;text-overflow:ellipsis;white-space:nowrap}.pending-file__copy small{margin-top:1px;color:rgb(var(--v-theme-on-surface-variant));font-size:.57rem}.composer-wrap>.v-progress-linear{margin-bottom:7px}.composer{padding-left:7px}.image-preview-header{display:flex;align-items:center;gap:10px;padding:14px 16px}.image-preview-header strong{overflow:hidden;font-size:.84rem;text-overflow:ellipsis;white-space:nowrap}.image-preview-body{display:grid;min-height:300px;place-items:center;padding:14px;background:rgba(var(--v-theme-on-surface),.035)}.image-preview-dialog{overflow:hidden}
.conversation-info-header{display:flex;align-items:center;gap:11px;padding:17px 18px;background:linear-gradient(135deg,rgba(var(--v-theme-primary),.055),transparent 60%)}.conversation-info-header__icon{display:grid;width:38px;height:38px;place-items:center;border-radius:11px;color:rgb(var(--v-theme-primary));background:rgba(var(--v-theme-primary),.11)}.conversation-info-header>div:nth-child(2){display:flex;min-width:0;flex-direction:column}.conversation-info-header strong{font-size:.9rem}.conversation-info-header small{margin-top:2px;color:rgb(var(--v-theme-on-surface-variant));font-size:.66rem}.conversation-info-body{display:grid;gap:18px;padding:18px}.conversation-info-loading{min-height:360px}.conversation-info-summary{display:grid;grid-template-columns:auto minmax(0,1fr);align-items:center;gap:13px;padding:14px;border-radius:14px;background:rgba(var(--v-theme-primary),.055)}.conversation-info-summary>div{display:flex;min-width:0;flex-direction:column}.conversation-info-summary strong{overflow:hidden;font-size:.92rem;text-overflow:ellipsis;white-space:nowrap}.conversation-info-summary span{margin-top:3px;color:rgb(var(--v-theme-on-surface-variant));font-size:.67rem}.conversation-info-metrics{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px}.conversation-info-metrics>div{display:grid;justify-items:center;gap:2px;padding:11px 8px;border:1px solid rgba(var(--v-theme-on-surface),.075);border-radius:12px;background:rgba(var(--v-theme-on-surface),.022)}.conversation-info-metrics .v-icon{margin-bottom:3px;color:rgb(var(--v-theme-primary))}.conversation-info-metrics strong{font-size:.9rem}.conversation-info-metrics span{color:rgb(var(--v-theme-on-surface-variant));font-size:.6rem}.conversation-info-section{display:grid;gap:9px}.conversation-info-section__title{display:flex;align-items:center;justify-content:space-between}.conversation-info-section__title strong{font-size:.78rem}.conversation-info-section__title span{display:grid;min-width:22px;height:22px;place-items:center;border-radius:999px;color:rgb(var(--v-theme-primary));background:rgba(var(--v-theme-primary),.1);font-size:.6rem;font-weight:750}.conversation-participants,.conversation-shared-files{display:grid;gap:5px}.conversation-participant{display:grid;grid-template-columns:auto minmax(0,1fr) auto;align-items:center;gap:10px;padding:8px;border-radius:11px}.conversation-participant:hover{background:rgba(var(--v-theme-on-surface),.035)}.conversation-participant>div{display:flex;min-width:0;flex-direction:column}.conversation-participant strong{overflow:hidden;font-size:.73rem;text-overflow:ellipsis;white-space:nowrap}.conversation-participant span{margin-top:2px;color:rgb(var(--v-theme-on-surface-variant));font-size:.61rem}.conversation-shared-file{display:grid;width:100%;grid-template-columns:auto minmax(0,1fr) auto;align-items:center;gap:10px;padding:9px;border:1px solid transparent;border-radius:11px;color:rgb(var(--v-theme-on-surface));background:rgba(var(--v-theme-on-surface),.025);text-align:left;cursor:pointer;transition:border-color .15s ease,background-color .15s ease}.conversation-shared-file:hover{border-color:rgba(var(--v-theme-primary),.18);background:rgba(var(--v-theme-primary),.055)}.conversation-shared-file>span{display:grid;width:35px;height:35px;place-items:center;border-radius:9px;color:rgb(var(--v-theme-primary));background:rgba(var(--v-theme-primary),.1)}.conversation-shared-file>div{display:flex;min-width:0;flex-direction:column}.conversation-shared-file strong{overflow:hidden;font-size:.69rem;text-overflow:ellipsis;white-space:nowrap}.conversation-shared-file small{margin-top:2px;color:rgb(var(--v-theme-on-surface-variant));font-size:.58rem}.conversation-info-dialog{overflow:hidden}
