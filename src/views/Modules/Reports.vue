    ...
    <div class="d-flex align-center justify-space-between flex-wrap ga-3 mb-5">
      <div>
        ...
        <p class="text-body-2 text-medium-emphasis mb-0">
          Run, save, and export operational workforce reports.
        </p>
      </div>
      <v-btn
        color="primary"
    ...
    <v-row>
      <v-col cols="12" :md="canManage ? 9 : 12">
        ...
        <v-card v-if="report">
          <v-card-title>
            {{ report.title }}
            <v-chip size="small" class="ml-2">{{ report.rows.length }} rows</v-chip>
          </v-card-title>
          <v-divider />
          <v-table>
            <thead>
...
const savedReportSubtitle = (item: any) => {
  const range = item.filters?.from && item.filters?.to
...
const save = async () => {
  ...
  try {
    await axios.post("/reports/saved", {
      ...
      delivery_period_days: deliveryFrequency.value ? deliveryPeriodDays.value : undefined,
      delivery_recipients: deliveryFrequency.value
        ? deliveryRecipients.value.split(",").map((email) => email.trim()).filter(Boolean)
        : undefined,
    });
    saveName.value = "";
...
<style scoped>
.report-type { min-width: 230px; max-width: 290px; }
.report-date { max-width: 190px; }
</style>
