<template>
  <v-container fluid class="dashboard-page pa-0">
    <section class="welcome-panel">
      ...
      <div class="daily-focus" :class="{ 'daily-focus--loading': calendarLoading && !dailyQuote.text }">
        <v-icon icon="mdi-format-quote-open" size="25" />
        <div v-if="dailyQuote.text"><span>Daily {{ dailyQuote.theme }}</span><blockquote>{{ dailyQuote.text }}</blockquote></div>
    ...
    <div class="dashboard-grid">
      ...
      <aside class="dashboard-sidebar">
        <section class="agenda-card">
          <header><div><span>Selected day</span><strong>{{ selectedDayLabel }}</strong></div><v-chip size="x-small" variant="tonal">{{ selectedDayEvents.length }} item{{ selectedDayEvents.length === 1 ? '' : 's' }}</v-chip></header>
          <div v-if="selectedDayEvents.length" class="agenda-list">
            <button v-for="event in selectedDayEvents" :key="event.id" type="button" @click="openCalendarEvent(event)">
              <i :style="{ background: event.color }" />
              <div><strong>{{ event.title }}</strong><span>{{ event.all_day ? 'All day' : eventTime(event.starts_at) }} · {{ eventLabel(event.type) }}</span></div>
              <v-icon icon="mdi-chevron-right" size="18" />
            </button>
          </div>
          <div v-else class="agenda-empty"><v-icon icon="mdi-calendar-check-outline" /><span>No scheduled items.</span></div>
        </section>

        <section class="announcements-card">
          <header><div><span>Company updates</span><strong>Announcements</strong></div><v-icon icon="mdi-bullhorn-outline" color="primary" /></header>
          <div v-if="calendarLoading && !announcements.length"><v-skeleton-loader type="list-item-two-line@3" /></div>
          <div v-else-if="announcements.length" class="announcement-list">
            <button v-for="announcement in announcements" :key="announcement.id" type="button" @click="openAnnouncement(announcement)">
              <span>{{ formatDate(announcement.published_at) }}</span><div><strong>{{ announcement.title }}</strong><small>{{ announcementExcerpt(announcement.content) }}</small></div>
            </button>
          </div>
          <div v-else class="agenda-empty"><v-icon icon="mdi-bullhorn-outline" /><span>No announcements yet.</span></div>
        </section>
      </aside>
    </div>
    ...
    <v-dialog v-model="announcementDialog" max-width="700">
      <v-card v-if="selectedAnnouncement" rounded="xl"><v-card-title class="dialog-title"><v-avatar color="primary" variant="tonal"><v-icon icon="mdi-bullhorn-outline" /></v-avatar><div><strong>{{ selectedAnnouncement.title }}</strong><small>{{ formatDate(selectedAnnouncement.published_at) }}</small></div><v-spacer /><v-btn icon="mdi-close" variant="text" @click="announcementDialog = false" /></v-card-title><v-divider /><v-card-text><div class="announcement-content" v-html="selectedAnnouncement.content" /></v-card-text></v-card>
    </v-dialog>

    <v-dialog v-model="eventDialog" max-width="560">
      <v-card v-if="selectedCalendarEvent" rounded="xl"><v-card-title class="dialog-title"><v-avatar :color="eventColorName(selectedCalendarEvent.type)" variant="tonal"><v-icon :icon="eventIcon(selectedCalendarEvent.type)" /></v-avatar><div><strong>{{ selectedCalendarEvent.title }}</strong><small>{{ eventDateLabel(selectedCalendarEvent) }}</small></div><v-spacer /><v-btn icon="mdi-close" variant="text" @click="eventDialog = false" /></v-card-title><v-divider /><v-card-text class="event-detail"><div><span>Type</span><strong>{{ eventLabel(selectedCalendarEvent.type) }}</strong></div><div v-if="selectedCalendarEvent.raw?.subtitle"><span>Details</span><strong>{{ selectedCalendarEvent.raw.subtitle }}</strong></div></v-card-text><v-card-actions v-if="selectedCalendarEvent.type === 'meeting'" class="px-5 pb-5"><v-spacer /><v-btn color="primary" class="text-none" prepend-icon="mdi-office-building-marker-outline" @click="$router.push({ name: 'workplace-hub' })">Open Workplace Hub</v-btn></v-card-actions></v-card>
    </v-dialog>
  </v-container>
</template>
...
function eventLabel(type?: string) { return ({ meeting: "Meeting", leave: "Approved leave", announcement: "Announcement" } as any)[type ?? ""] ?? "Calendar item"; }
function eventIcon(type?: string) { return ({ meeting: "mdi-account-group-outline", leave: "mdi-calendar-account-outline", announcement: "mdi-bullhorn-outline" } as any)[type ?? ""] ?? "mdi-calendar-outline"; }
function eventColorName(type?: string) { return ({ meeting: "primary", leave: "secondary", announcement: "warning" } as any)[type ?? ""] ?? "primary"; }
